	...
	return new Promise((resolve, reject) => {
		...
		.then((manifest) => {
			return new Promise((resolve, reject) => {
				...
				var prevZip = getPreviousZip(paths.distGwdPath() + zipName)
				.then((prevZip) => {
					var promises = [];
						...
						['js', 'css'].forEach((format) => {
							if (format in manifest.files && Array.isArray(manifest.files[format])) {
								manifest.files[format].forEach((filePath, i) => {
									...
									const safeName = getGwdPackageFilename(prevZip, filePath);
									// build new zip
...
function getGwdPackageFilename(prevZip, filePath) {
	// determine base-name and unique-name
	const basename = path.basename(filePath).replace(/\.[^\.]*$/, '');
	const uniqueName = `${basename}__${uuid().slice(-4)}${path.extname(filePath)}`;

	// check all the filenames for match to filePath
	if (prevZip) {
		for (var filename in prevZip.files) {
			const regex = new RegExp(`${escapeRegex(basename)}__.{4}${escapeRegex(path.extname(filePath))}`);
			// return existing matching filename
