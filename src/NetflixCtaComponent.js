    ...
    var component = Object.create(HTMLElement.prototype, {
                    ...
                    if (MonetComponent) {
                        MonetComponent.getMonetData().then(
                            function (data) {
                                ...
                                } else {// handle dynamic key set manually
                                ...
                                try {
                                    ...
                                    MonetComponent.getBackupMonetData().then(
                                        function (backupData) {
                                            ...
                                            if (!backupData) {
                                                console.error('No backupData found. Please check that your monet-gwd component has a `dynamic-feed-sheet-name` attribute');
                                                return;
                                            }
        ...
        resize: {
            value: function (w, h) {
                ...
                var widthTransform  = (bbb.width * .8) / bb.width;
                var heightTransform = (bbb.height * .8) / bb.height;
                ...
                if (this.hasArrow) {
                    ...
                    var ht  = (bbb.height * .5) / abb.height;
                    ...
                    this.arrow.setAttribute("style", "transform: scale(" + ht.toFixed(3) + "); top: " + (ay - 1) + "px; right: " + ax + "px;");
                }

                this.copy.setAttribute("style", "transform: translateZ(0) scale(" + value.toFixed(3) + "); left: " + xp + "px; top:" + yp + "px;)");
            }
        },
        ...
        _style: {
            value: function () {
                this._addStyle('.button',
                    'cursor: pointer;overflow: hidden;line-height: 40px;text-align: center;font-size:' + this.data.size + 'px; font-family: ' + this.data.font);

                this._addStyle('.button .fill',
                ...
                this._addStyle('.button .arrow',
                    'transform-origin: top right;top:0;left:auto;right:0;line-height:40px;font-size: 230% !important;margin-top:-1px;transition: color .4s cubic-bezier(0.19, 1, 0.22, 1);color:' + this.data.color[1]);

                if (!this.isMobile) {

                    this._addStyle('.button:hover .fill',
                        'transform: translate(0, 0); -webkit-transform: translate(0, 0);');

                    this._addStyle('.button.hover .fill',
                        'transform: translate(0, 0); -webkit-transform: translate(0, 0);');

                    this._addStyle('.button:hover .arrow',
                        'color:' + this.data.color[0]);

                    this._addStyle('.button.hover .arrow',
                        'color:' + this.data.color[0]);

                    this._addStyle('.button:hover .copy',
                        'color:' + this.data.color[0]);
                ...
                this._addStyle('.button .copy',
                    'transform-origin: 0 0;white-space:nowrap;letter-spacing:1.5px;text-align:center;padding: 4px 8%;transition: color .4s cubic-bezier(0.19, 1, 0.22, 1);color:' + this.data.color[1]);

                this._addStyle('.button .border',
